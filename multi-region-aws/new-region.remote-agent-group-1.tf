...
variable "aws_group_1_remote_agent_az" { 
  default = "a"
}

variable "num_of_remote_private_agents_group_1" {
  default = "1"
}

# Create a subnet to launch slave private node into
resource "aws_subnet" "group_1_private" {
...
resource "aws_instance" "group_1_remote_agent" {
  ...
  count = "${var.num_of_remote_private_agents_group_1}"
...
resource "null_resource" "group_1_remote_agent" {
  # Changes to any instance of the cluster requires re-provisioning
  triggers {
    cluster_instance_ids = "${null_resource.bootstrap.id}"
    current_ec2_instance_id = "${aws_instance.group_1_remote_agent.*.id[count.index]}"
  }
  # Bootstrap script can run on any instance of the cluster
  # So we just choose the first in this case
  connection {
    host = "${element(aws_instance.group_1_remote_agent.*.public_ip, count.index)}"
    user = "${module.aws-tested-oses-bursted.user}"
  }
...
#  value = "${join(", ", aws_instance.group_1_remote_agent.*.public_ip)}"
#}
