...
resource "aws_subnet" "default_group_2_public" {
  ...
  availability_zone       = "${var.aws_region}${var.aws_group_2_public_agent_az}"
}
...
resource "null_resource" "public-agent-group-2" {
  # Changes to any instance of the cluster requires re-provisioning
  triggers {
    cluster_instance_ids = "${null_resource.bootstrap.id}"
    current_ec2_instance_id = "${aws_instance.public-agent-group-2.*.id[count.index]}"
  }

  # Bootstrap script can run on any instance of the cluster
  # So we just choose the first in this case
  connection {
    host = "${element(aws_instance.public-agent-group-2.*.public_ip, count.index)}"
    user = "${module.aws-tested-oses.user}"
  }
...
#  value = ["${aws_instance.public-agent-group-2.*.public_ip}"]
#}
